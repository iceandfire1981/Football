...
public class OddDataHolder {
    ...
    private static final String QUERY_ODD_RECORD = "select * from odd_data where team_id = ? order by match_datetime desc";
    ...
    private void processTeamData(){
        HashMap<Integer, ArrayList<OddDataInfo>> data_map = new HashMap<>();
        for(int index = 0; index < mAllOddRecords.size(); index++) {
            ...
            if (null != process_odd_list && process_odd_list.size() > 0) {
                for(int process_index = 0; process_index < process_odd_list.size(); process_index++){
                    OddDataInfo current_odd_panlu_data = process_odd_list.get(index);
                    int target_input_monkey = input_money * (process_index + 1);
                    int target_finally_money = input_money * (process_index + 1);
    ...
    private ArrayList<OddDataInfo> createProcessDataList(int current_index, ArrayList<OddDataInfo> source_records, String target_panlu) {
        ...
        //获取字串，从索引指示的位置开始直到结束。
        List<OddDataInfo> sub_source_data_list = source_records.subList(current_index, (source_records.size() - 1));
        ...
        List<OddDataInfo> finally_sub_data_list = sub_source_data_list.subList(current_index, (finally_total_size - 1));
